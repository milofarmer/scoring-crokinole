...
const CONFIG = { pointsWin: 2, pointsTie: 1 } as const;

/**
 * A four-team round robin engineered so that two pairs finish level on points
 * and 20's: teams 1 and 2 lead on 4, teams 3 and 4 trail on 2.
...
const ROUND_ROBIN: readonly Match[] = [
  played(1, 1, 2, 90, 100), // 2 beats 1
  played(2, 3, 4, 90, 100), // 4 beats 3
  played(3, 1, 3, 100, 90), // 1 beats 3
  played(4, 2, 4, 100, 90), // 2 beats 4
  played(5, 1, 4, 100, 90), // 1 beats 4
  played(6, 2, 3, 90, 100), // 3 beats 2
];

test('aggregates wins, ties, losses and points', () => {
  const table = computeStandings(TEAMS, ROUND_ROBIN, CONFIG);
  const one = table.get(1);
  assert.ok(one);
  assert.equal(one.played, 3);
  assert.equal(one.wins, 2);
  assert.equal(one.losses, 1);
  assert.equal(one.points, 4);
  assert.equal(table.get(2)?.points, 4);
  assert.equal(table.get(3)?.points, 2);
  assert.equal(table.get(4)?.points, 2);
});

test('head-to-head decides FIRST place, but is not applied further down', () => {
  const ranked = rankPoule(TEAMS, ROUND_ROBIN, CONFIG, 1);
...
test("20's outrank head-to-head: more 20's takes first even after losing the meeting", () => {
  ...
  const ranked = rankPoule(TEAMS, withTwenties, CONFIG, 1);
...
test('a bye counts as a win, and knockout matches never touch the poule table', () => {
  ...
  const table = computeStandings(teams, matches, CONFIG);
  assert.equal(table.get(1)?.points, 2, 'the bye is worth a win');
...
test('teams with no results yet still appear, on zero', () => {
  const ranked = rankPoule([makeTeam(9, 3)], [], CONFIG, 3);
...
test('only the requested poule is ranked', () => {
  ...
  const ranked = rankPoule(teams, [], CONFIG, 2);
